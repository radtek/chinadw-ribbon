...
namespace ARM_User.ServiceLayer.Reporting.Excel
{
     ...
     * 7 Сведения о сроках представления эмитентами отчетов об итогах размещения исламских ценных бумаг
     */
    public class RepICB : ReportToExcel
    {
        #region Fields
       ...
       protected DateTime dateBegin;//, dateEnd;
        ...
        public RepICB(RepForm form, LanguageIds language)
        {
            this.form = form;
        ...
        public RepICB(RepForm form, LanguageIds language, DateTime dateBegin,/* DateTime dateEnd,*/ Decimal idRegion, Decimal cancelConfirmRep)
            : this(form, language)
        {
        ...
        protected virtual void CreateReportFromXml()
        {
            ...
            try
            {
                ...
                ws.Range["B3"].Value = " Сведения о сроках представления эмитентами отчетов об итогах размещения исламских ценных бумаг на дату " + dateBegin;
                ...
                for (var i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    (rngDataRow.Cells[rowIndex, 7] as Range).Value2 = dt.Rows[i]["DATE_REP_TERM_PLA"].ToString();
                    (rngDataRow.Cells[rowIndex, 8] as Range).Value2 = dt.Rows[i]["DATE_CONFIRM_PRV_REP"].ToString();
                    (rngDataRow.Cells[rowIndex, 9] as Range).Value2 = dt.Rows[i]["DATE_REP_PAY"].ToString();
                    (rngDataRow.Cells[rowIndex, 10] as Range).Value2 = Convert.ToInt32(dt.Rows[i]["COUNT_ICB"]);
                    (rngDataRow.Cells[rowIndex, 11] as Range).Value2 = Convert.ToInt32(dt.Rows[i]["COUNT_PL"]);
                    (rngDataRow.Cells[rowIndex, 12] as Range).Value2 = Convert.ToInt32(dt.Rows[i]["VALUE_CAPITAL"]);
                    (rngDataRow.Cells[rowIndex, 13] as Range).Value2 = dt.Rows[i]["NAME_PLACEMENT"].ToString();
                    (rngDataRow.Cells[rowIndex, 14] as Range).Value2 = dt.Rows[i]["NAME_REGION"].ToString();
                    (rngDataRow.Cells[rowIndex, 15] as Range).Value2 = dt.Rows[i]["BIN"].ToString();
