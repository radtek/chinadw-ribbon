...
namespace ARM_User.ServiceLayer.Reporting.Excel
{
     ...
     * 42.	Форма А-15. Список объявленных ценных бумаг по состоянию на дату	
     */
    public class RepListAuthorizedSecur : ReportToExcel
    {
        #region Fields
        ...
        #region Constructors
        public RepListAuthorizedSecur()
        {
        }

        public RepListAuthorizedSecur(RepForm form, LanguageIds language)
        {
            this.form = form;
        ...
        public RepListAuthorizedSecur(RepForm form, LanguageIds language, DateTime dateBegin)
            : this(form, language)
        {
        ...
        public System.Data.DataTable Getcursor()
        {
            ...
            using (var cmd = ConnectionHolder.Instance.Connection.CreateCommand())
            {
                ...
                cmd.CommandText = "G_REP_LIST_AUTHORIZED_SECUR";
        ...
        protected virtual void CreateReportFromXml()
        {
            ...
            try
            {
                ...
                if (InitApplication.CurrentLangId == LanguageIds.Russian) 
                    ws.Range["A2"].Value = "Форма А-15. Список объявленных ценных бумаг по состоянию на " + dateBegin.ToShortDateString() + " года.";
                if (InitApplication.CurrentLangId == LanguageIds.Kazakh)
                    ws.Range["A2"].Value = "Жарияланған бағалы қағаздардың тізімі " + dateBegin.ToShortDateString() + " ж. жағдайы бойынша";
                ...
                for (var i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    (rngDataRow.Cells[rowIndex, 3] as Range).Value2 = dt.Rows[i]["NAME_EMIT"].ToString();
                    (rngDataRow.Cells[rowIndex, 4] as Range).Value2 = dt.Rows[i]["NAME_SPEC_S"].ToString();
                    (rngDataRow.Cells[rowIndex, 5] as Range).Value2 = dt.Rows[i]["NAME_SBC"].ToString();
                    //(rngDataRow.Cells[rowIndex, 6] as Range).Value2 = Convert.ToDateTime(dt.Rows[i]["DATLAST"]);
                    (rngDataRow.Cells[rowIndex, 6] as Range).Value2 = dt.Rows[i]["DATLAST"].ToString();
                    ...
                    (rngDataRow.Cells[rowIndex, 9] as Range).Value2 = dt.Rows[i]["COUNT_DECLARED_SHARE"].ToString();
                    (rngDataRow.Cells[rowIndex, 10] as Range).Value2 = dt.Rows[i]["DIVIDEND_GRANT_SIZE"].ToString();
                ...
                XmlExcelProperty[] plist =
                    {
                      ...
                      new XmlExcelProperty("NAME_SBC", XmlExcelProperty.XmlType.xmlString),
                      new XmlExcelProperty("DATLAST", XmlExcelProperty.XmlType.xmlString),
                      new XmlExcelProperty("NIN", XmlExcelProperty.XmlType.xmlString),
                      new XmlExcelProperty("ISIN", XmlExcelProperty.XmlType.xmlString),       
                      new XmlExcelProperty("NAME_CATEGORY", XmlExcelProperty.XmlType.xmlString),                   
                      new XmlExcelProperty("COUNT_DECLARED_SHARE", XmlExcelProperty.XmlType.xmlString),                   
                      new XmlExcelProperty("DIVIDEND_GRANT_SIZE", XmlExcelProperty.XmlType.xmlString),                   
