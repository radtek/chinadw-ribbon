...
namespace ARM_User.ServiceLayer.Reporting.Excel
{
     ...
     * 65.	Форма И-10. Перечень юридических лиц, осуществивших государственную регистрацию выпусков исламских ценных бумаг, за период
     */
    public class RepFormI10 : ReportToExcel
    {
        #region Fields
        ...
        public RepFormI10(RepForm form, LanguageIds language)
        {
            this.form = form;
        ...
        public RepFormI10(RepForm form, LanguageIds language, DateTime dateBegin, DateTime dateEnd)
            : this(form, language)
        {
        ...
        public System.Data.DataTable Getcursor()
        {
            ...
            using (var cmd = ConnectionHolder.Instance.Connection.CreateCommand())
            {
                ...
                cmd.CommandText = "G_REP_FORM_I10";
        ...
        protected virtual void CreateReportFromXml()
        {
            ...
            try
            {
                ...
                if (InitApplication.CurrentLangId == LanguageIds.Russian) 
                ws.Range["A2"].Value = "Форма И-10. Перечень юридических лиц, осуществивших государственную регистрацию выпусков исламских ценных бумаг, за период c " + dateBegin.ToShortDateString() + " г. по " + dateEnd.ToShortDateString() + " г.";
                else if (InitApplication.CurrentLangId == LanguageIds.Kazakh)
                    ws.Range["A2"].Value = "Ислам бағалы қағаздар шығарылымдарды мемлекеттік тіркеуді жүзеге асырған заңды тулғалардың тізімі " + dateBegin.ToShortDateString() + " ж. және " + dateEnd.ToShortDateString() + " ж. жағдайы бойынша";
                ...
                for (var i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    (rngDataRow.Cells[rowIndex, 11] as Range).Value2 = dt.Rows[i]["COUNT_ICB"].ToString();
                    (rngDataRow.Cells[rowIndex, 12] as Range).Value2 = dt.Rows[i]["RAZMER"].ToString();
                    (rngDataRow.Cells[rowIndex, 13] as Range).Value2 = dt.Rows[i]["NAME_KND"].ToString();
                    (rngDataRow.Cells[rowIndex, 14] as Range).Value2 = dt.Rows[i]["DESIGNATION_NIN"].ToString();
                    (rngDataRow.Cells[rowIndex, 15] as Range).Value2 = dt.Rows[i]["ISIN"].ToString();  
                    (rngDataRow.Cells[rowIndex, 16] as Range).Value2 = dt.Rows[i]["ORIGINATOR"].ToString();
                    (rngDataRow.Cells[rowIndex, 17] as Range).Value2 = dt.Rows[i]["NAME_REGISTRARS"].ToString();
                    (rngDataRow.Cells[rowIndex, 18] as Range).Value2 = dt.Rows[i]["COMMENTS_REPAYMENT"].ToString();
                    (rngDataRow.Cells[rowIndex, 19] as Range).Value2 = dt.Rows[i]["COMMENTS_REDEMPTION"].ToString();
                    (rngDataRow.Cells[rowIndex, 20] as Range).Value2 = dt.Rows[i]["EVENTS"].ToString();
                    (rngDataRow.Cells[rowIndex, 21] as Range).Value2 = dt.Rows[i]["COMMENTS"].ToString();                    
                    rowIndex++;
                }
