...
namespace ARM_User.ServiceLayer.Reporting.Excel
{
     ...
     * 59.	Форма КДР-13.  Перечень эмитентов, аннулировавших выпуски казахстанских депозитарных расписок за период c __.__.____г. по __.__.____г.
     */
    public class RepForm13KDRAnnul : ReportToExcel
    {
        #region Fields
        ...
        #region Constructors
        public RepForm13KDRAnnul()
        {
        }

        public RepForm13KDRAnnul(RepForm form, LanguageIds language)
        {
            this.form = form;
        ...
        public RepForm13KDRAnnul(RepForm form, LanguageIds language, DateTime dateBegin, DateTime dateEnd)
            : this(form, language)
        {
        ...
        public System.Data.DataTable Getcursor()
        {
            ...
            using (var cmd = ConnectionHolder.Instance.Connection.CreateCommand())
            {
                ...
                cmd.CommandText = "G_REP_FORM_13_KDR_ANNUL";
        ...
        protected virtual void CreateReportFromXml()
        {
            ...
            try
            {
                ...
                if (InitApplication.CurrentLangId == LanguageIds.Russian)
                {
                    ...
                    ws.Range["B2"].Value = "Форма КДР-13.  Перечень эмитентов, аннулировавших выпуски казахстанских депозитарных расписок за период c " + dateBegin.ToShortDateString() + " г. по " + dateEnd.ToShortDateString() + " г.";
                    ws.Range["B2", "H2"].MergeCells = true;
                    ws.Range["B2"].HorizontalAlignment = Microsoft.Office.Interop.Excel.XlHAlign.xlHAlignCenter;
                }
                else
                {
                    ws.Range["B2"].Font.Bold = true;
                    ws.Range["B2"].Value = "Қазақстандық депозитарлы қолхаттар шығарылымды жойған эмитенттердің тізімі " + dateBegin.ToShortDateString() + " ж. - " + dateEnd.ToShortDateString() + "ж.";
                    ws.Range["B2", "H2"].MergeCells = true;
                    ws.Range["B2"].HorizontalAlignment = Microsoft.Office.Interop.Excel.XlHAlign.xlHAlignCenter;
                }
                ...
                for (var i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    (rngDataRow.Cells[rowIndex, 5] as Range).Value2 = dt.Rows[i]["BIN"].ToString();
                    ...
                    (rngDataRow.Cells[rowIndex, 9] as Range).Value2 = Convert.ToInt32(dt.Rows[i]["COUNT_DEPRECEIPT"]);
                    (rngDataRow.Cells[rowIndex, 10] as Range).Value2 = dt.Rows[i]["NAMERU_FULL"].ToString();
