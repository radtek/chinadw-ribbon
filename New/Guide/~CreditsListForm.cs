...
namespace ARM_User.New.Guide
{
    public partial class CreditsListForm : BSB.Common.MDIChildForm
    {
        ...
        {
            dpPayment.Text = String.Format("График оплаты - {0}",getCurrentName("tsCredits", "CUSTOMER_NAME"));
        ...
        private void createManifestXML(int type, String mfile)
        {
                ...
                using (XtraSaveFileDialog saveFileDialog1 = new XtraSaveFileDialog())
                {
                    ...
                    if (saveFileDialog1.ShowDialog() == DialogResult.OK)
                    {
                        try
                        {
                            ...
                            createDataXML(type, sfileName);
                            createManifestXML(type, mFileName);
                            AddZip(type, sfileName, mFileName);
                        }
                        catch (Exception oe)
                        {
        ...
        private String getXML(int type_xml, DateTime p_date_)
        {
            String result;
            using (OracleCommand cmd = dmControler.frmMain.oracleConnection.CreateCommand())
            {
                try
                {
                    ...
                    cmd.Parameters.Add("type_xml", OracleDbType.Decimal, type_xml, ParameterDirection.Input);
        ...
        private void createDataXML(int type, String path)
        {
            String XML=String.Empty;
            String s = beData.EditValue.ToString();
            DateTime FilterDateTime = Convert.ToDateTime(s);
           
            XML = getXML(type, FilterDateTime.Date);
        ...
        private void AddZip(int type, String sfileName, String mFileName)
        {
            ...
            String zipFileName = String.Format("{0}-[{1}]-{2}.zip", stype, FilterDateTime.Date.ToString("dd-MM-yyyy"), DateTime.Now.ToString("HHmmss"));
