...
namespace ARM_User.ServiceLayer.Reporting.Excel
{
     ...
     * 83.	Форма А-25. Список управляющих компаний по состоянию на
     */
    public class RepFormA25 : ReportToExcel
    {
        #region Fields
        ...
        public RepFormA25(RepForm form, LanguageIds language)
        {
            this.form = form;
        ...
        public RepFormA25(RepForm form, LanguageIds language, DateTime dateBegin)
            : this(form, language)
        {
        ...
        public System.Data.DataTable Getcursor()
        {
            ...
            using (var cmd = ConnectionHolder.Instance.Connection.CreateCommand())
            {
                ...
                cmd.CommandText = "G_REP_FORM_A25";
        ...
        protected virtual void CreateReportFromXml()
        {
            ...
            try
            {
                ...
                if (InitApplication.CurrentLangId == LanguageIds.Russian)
                    ws.Range["A2"].Value = "Форма А-25. Список управляющих компаний по состоянию на " + dateBegin.ToShortDateString() + " г.";
                else if (InitApplication.CurrentLangId == LanguageIds.Kazakh)
                    ws.Range["A2"].Value = "Форма А-25. Басқару компаниялардың тізімі " + dateBegin.ToShortDateString() + " ж. жағдайы бойынша";
                ...
                for (var i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    (rngDataRow.Cells[rowIndex, 2] as Range).Value2 = dt.Rows[i]["REG_DATE"].ToString();
                    (rngDataRow.Cells[rowIndex, 3] as Range).Value2 = dt.Rows[i]["REREG_DATE"].ToString();
                    ...
                    (rngDataRow.Cells[rowIndex, 6] as Range).Value2 = dt.Rows[i]["NAME_EMIT"].ToString();
                    (rngDataRow.Cells[rowIndex, 7] as Range).Value2 = dt.Rows[i]["ADDRESS"].ToString();
                    (rngDataRow.Cells[rowIndex, 8] as Range).Value2 = dt.Rows[i]["LICENSE_CONTR"].ToString();
                    (rngDataRow.Cells[rowIndex, 9] as Range).Value2 = dt.Rows[i]["NAMERU_SHORT"].ToString();
