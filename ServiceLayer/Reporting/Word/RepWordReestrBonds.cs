...
{
    public class RepWordReestrBonds : GeneralWordReport
    {
        #region Fields
        protected Microsoft.Office.Interop.Excel.Range range2;
        protected Decimal languageId;
#pragma warning disable CS0108 // 'RepWordReestrBonds.wordparagraph' hides inherited member 'WordReport.wordparagraph'. Use the new keyword if hiding was intended.
#pragma warning disable CS0169 // The field 'RepWordReestrBonds.wordparagraph' is never used
        private Microsoft.Office.Interop.Word.Paragraph wordparagraph;
#pragma warning restore CS0169 // The field 'RepWordReestrBonds.wordparagraph' is never used
#pragma warning restore CS0108 // 'RepWordReestrBonds.wordparagraph' hides inherited member 'WordReport.wordparagraph'. Use the new keyword if hiding was intended.
        ...
        protected string Srestinfo, SregDateUL, SdateReg, ScountHolders, ScountHoldersNonres, STotalcountHolders, STextAddInfo, advrpmcond, priorredcond;

        protected string TableName1, TableName2, TableName3, TableName4, TableName5, TableName6, TableName7, Stext, STextNum, SNumSerial, STextTable, TextTable;
        protected int coloumn, coloumntext;
        protected string[] template = new string[40];
        public RepWordReestrBonds(RepForm form, LanguageIds language, decimal? idBond)
            : base(form, language)
        {
            this.form = form;
            this.idBond = idBond;
        ...
        protected virtual string GetData()
        {
            using (var cmd = ConnectionHolder.Instance.Connection.CreateCommand())
            {
                ...
                cmd.Parameters.Add("Advrpmcond_", OracleDbType.Varchar2, ParameterDirection.Output);
                cmd.Parameters.Add("Priorredcond_", OracleDbType.Varchar2, ParameterDirection.Output);
                ...
                cmd.Parameters["Name_Bondknd_"].Size = 8000;
                cmd.Parameters["NIN_"].Size = 8000;
                ...
                cmd.Parameters["Holder_Name_"].Size = 8000;
                cmd.Parameters["Pagent_Name_"].Size = 8000;

                cmd.Parameters["Name_Audit_"].Size = 8000;
                cmd.Parameters["Origin_"].Size = 8000;
                cmd.Parameters["Date_Annul_"].Size = 8000;
                cmd.Parameters["Comments_"].Size = 8000;
                cmd.Parameters["Off_Name_"].Size = 8000;

                cmd.Parameters["Idrest_"].Size = 4000;
                cmd.Parameters["Idadvrpm_"].Size = 4000;
                cmd.Parameters["Idpriorred_"].Size = 4000;
                cmd.Parameters["Eventdefault_"].Size = 8000;
                cmd.Parameters["Restinfo_"].Size = 8000;
                cmd.Parameters["Count_Holders_"].Size = 4000;
                cmd.Parameters["Count_Holders_Nonres_"].Size = 4000;
                cmd.Parameters["Advrpmcond_"].Size = 8000;
                cmd.Parameters["Priorredcond_"].Size = 8000;
                ...
                advrpmcond= cmd.Parameters["Advrpmcond_"].Value.ToString();
                if (advrpmcond == "null")
                {
                    advrpmcond = string.Empty;
                }
                priorredcond = cmd.Parameters["Priorredcond_"].Value.ToString();
                if (priorredcond == "null")
                {
                    priorredcond = string.Empty;
                }
        ...
        public override void ShowReport()
        {
            ...
            if (language == LanguageIds.Russian)
            {
                ...
                STextAddInfo = "Дополнительные сведения о выпуске:";
                ...
                template[35] = "Общее количество размещенных облигаций";
                ...
                template[25] = "№ п/п";
                template[26] = "Наименование учредителя/акционера";
                template[27] = "ИИН/БИН/ОКПО";
                template[28] = "Вид, номер, дата документа";
                template[29] = "Процентное соотношение голосующих акций";

                TableName7 = "Сведения о должностных лицах эмитента:";
                template[30] = "Код";
                template[31] = "Должность";
                template[32] = "Фамилия, имя, при наличии отчества";                
                template[34] = "Процентное соотношение акций, принадлежащих должностному лицу, к общему количеству размещенных акций";
            ...
            else
            {
                ...
                Srestinfo = "Основные сведения о реструктуризации обязательств эмитента:";// + restinfo;
                STextAddInfo = "Мәселе туралы қосымша ақпарат алу үшін:";
                ...
                template[35] = "Орналастырылған облигациялар жалпы саны";                
            ...
            try
            {
                ...
                ReplaceText(wordApp, "${Origin}", origin);
                ...
                CreateTableWord(wordDoc, dt1.Rows.Count + 2, 6, 4, TableName1, true, true);
                ...
                CreateTableWord(wordDoc, 3, 2, 5, ScountHolders, true,false);
                coloumntext = 1;
                for (var i = 0; i < 2; i++)
                {
                    coloumntext = coloumntext + 1;
                    if (coloumntext == 2)
                    {
                        STextTable = STotalcountHolders;//  "Всего";
                        TextTable = countHolders;
                    }
                    else if (coloumntext == 3)
                    {
                        STextTable = ScountHoldersNonres; //"из них нерезидентов";
                        TextTable = countHoldersNonres;
                    }

                    wordcellrange = wordtable.Cell(coloumntext, 1).Range;
                    wordcellrange.Text = STextTable;
                    wordcellrange.Font.Bold = 0;
                    wordcellrange.Borders.Enable = 0;
                    wordcellrange.Font.Size = 11;
                    wordtable.Cell(coloumntext, 1).Range.ParagraphFormat.Alignment = WdParagraphAlignment.wdAlignParagraphLeft;

                    wordcellrange = wordtable.Cell(coloumntext, 2).Range;
                    wordcellrange.Text = TextTable;
                    wordcellrange.Font.Bold = 1;
                    wordcellrange.Borders.Enable = 0;
                    wordcellrange.Font.Size = 11;
                    wordtable.Cell(coloumntext, 2).Range.ParagraphFormat.Alignment = WdParagraphAlignment.wdAlignParagraphLeft;
                }
                ...
                //TableName2
                CreateTableWord(wordDoc, dt2.Rows.Count + 1, 4, 6, null, false, true);
                ...
                CreateTableWord(wordDoc, 5, 2, 7, STextAddInfo, true,false);                
                coloumn = 1;
                for (var i = 0; i < 4; i++)
                {
                    coloumn = coloumn + 1;                   
