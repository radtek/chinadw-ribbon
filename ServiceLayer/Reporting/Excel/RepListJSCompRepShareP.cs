...
namespace ARM_User.ServiceLayer.Reporting.Excel
{
     ...
     * 38.	Форма А-11.  Перечень акционерных обществ, утвердивших отчеты об итогах размещения акций за период 	
     */
    public class RepListJSCompRepShareP : ReportToExcel
    {
        #region Fields
        ...
        #region Constructors
        public RepListJSCompRepShareP()
        {
        }

        public RepListJSCompRepShareP(RepForm form, LanguageIds language)
        {
            this.form = form;
        ...
        public RepListJSCompRepShareP(RepForm form, LanguageIds language, DateTime dateBegin, DateTime dateEnd)
            : this(form, language)
        {
        ...
        public System.Data.DataTable Getcursor()
        {
            ...
            using (var cmd = ConnectionHolder.Instance.Connection.CreateCommand())
            {
                ...
                cmd.CommandText = "G_REP_LIST_JS_COMP_REP_SHARE_P";
        ...
        protected virtual void CreateReportFromXml()
        {
            ...
            try
            {
                ...
                var rngBelowDataRow = ws.Rows[rngDataRow.Row, Type.Missing] as Range;
                 ...
                 if (InitApplication.CurrentLangId == LanguageIds.Russian)
                     ws.Range["A2"].Value = "Форма А-11.  Перечень акционерных обществ, утвердивших отчеты об итогах размещения акций за период c " + dateBegin.ToShortDateString() + " г. по " + dateEnd.ToShortDateString() + " г.";
                 else if (InitApplication.CurrentLangId == LanguageIds.Kazakh)
                     ws.Range["A2"].Value = "Ацияларды орналастыру қорытындылары жөніндегі ережені бекіткен акционерлік қоғамдардың тізімі " + dateBegin.ToShortDateString() + " ж. және " + dateEnd.ToShortDateString() + " ж. жағдайы бойынша";
