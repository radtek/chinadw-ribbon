...
Section "Dummy Section" SecDummy
  ...
  CreateShortCut "$SMPROGRAMS\BSB\HDChina\User\ÀÈÏ HDChina.lnk" "$INSTDIR\ARM_User.exe" "" "" 0 SW_SHOWMAXIMIZED 
  CreateShortCut "$SMPROGRAMS\BSB\HDChina\Uninstall.lnk" "$INSTDIR\Uninstall.exe"
  
  CreateShortCut "$DESKTOP\ÀÐÌ «HDChina».lnk" "$INSTDIR\ARM_User.exe" "" "" 0 SW_SHOWMAXIMIZED 
 
  ;Store installation folder
  WriteRegStr HKCU "Software\BSB\HDChina\ARM_User" "" $INSTDIR
  
  WriteRegStr HKCU "Software\Microsoft\Windows\CurrentVersion\Uninstall\BSB_HDChina" "DisplayName" "ÀÈÏ HDChina"
