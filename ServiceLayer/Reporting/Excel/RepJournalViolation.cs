...
namespace ARM_User.ServiceLayer.Reporting.Excel
{
     ...
     * 77.	Журнал нарушений, за период
     */
    public class RepJournalViolation : ReportToExcel
    {
        #region Fields
        ...
        #region Constructors
        public RepJournalViolation()
        {
        }

        public RepJournalViolation(RepForm form, LanguageIds language)
        {
            this.form = form;
        ...
        public RepJournalViolation(RepForm form, LanguageIds language, DateTime dateBegin, DateTime dateEnd)
            : this(form, language)
        {
        ...
        public System.Data.DataTable Getcursor()
        {
            ...
            using (var cmd = ConnectionHolder.Instance.Connection.CreateCommand())
            {
                ...
                cmd.CommandText = "G_REP_JOURNAL_VIOLATION";
        ...
        protected virtual void CreateReportFromXml()
        {
            ...
            try
            {
                ...
                if (InitApplication.CurrentLangId == LanguageIds.Russian)
                    ws.Range["A2"].Value = "Журнал нарушений, за период c " + dateBegin.ToShortDateString() + " г. по " + dateEnd.ToShortDateString() + " г.";
                else if (InitApplication.CurrentLangId == LanguageIds.Kazakh)
                    ws.Range["A2"].Value = dateBegin.ToShortDateString() + " ж. және " + dateEnd.ToShortDateString() + " ж. кезең бойынша бұзушылықтар журналы";
                ...
                for (var i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    (rngDataRow.Cells[rowIndex, 2] as Range).Value2 = dt.Rows[i]["DATE_UST"].ToString();
                    (rngDataRow.Cells[rowIndex, 3] as Range).Value2 = dt.Rows[i]["NAME_EMIT"].ToString();
                    (rngDataRow.Cells[rowIndex, 4] as Range).Value2 = dt.Rows[i]["NAMERU_FULL"].ToString();
                    (rngDataRow.Cells[rowIndex, 5] as Range).Value2 = dt.Rows[i]["REGNUM"].ToString();
                    (rngDataRow.Cells[rowIndex, 6] as Range).Value2 = dt.Rows[i]["NAME_KND"].ToString();
        ...
        {
          new XmlExcelProperty("DATE_UST", XmlExcelProperty.XmlType.xmlString),      
          new XmlExcelProperty("NAME_EMIT", XmlExcelProperty.XmlType.xmlString),         
          new XmlExcelProperty("NAMERU_FULL", XmlExcelProperty.XmlType.xmlString),
          new XmlExcelProperty("REGNUM", XmlExcelProperty.XmlType.xmlString),
          new XmlExcelProperty("NAME_KND", XmlExcelProperty.XmlType.xmlString),
