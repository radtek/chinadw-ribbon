...
namespace ARM_User.ServiceLayer.Reporting.Excel
{
     ...
     * 85.	Перечень эмитентов , аннулировавших выпуски исламских ценных бумаг, за период c __.__.____г. по __.__.____г.
     */
    public class RepListIssuersAnnulISI : ReportToExcel
    {
        #region Fields
        ...
        #region Constructors
        public RepListIssuersAnnulISI()
        {
        }

        public RepListIssuersAnnulISI(RepForm form, LanguageIds language)
        {
            this.form = form;
        ...
        public RepListIssuersAnnulISI(RepForm form, LanguageIds language, DateTime dateBegin, DateTime dateEnd)
            : this(form, language)
        {
        ...
        public System.Data.DataTable Getcursor()
        {
            ...
            using (var cmd = ConnectionHolder.Instance.Connection.CreateCommand())
            {
                ...
                cmd.CommandText = "G_REP_LIST_ISSUERS_ANNUL_ISI";
        ...
        protected virtual void CreateReportFromXml()
        {
            ...
            try
            {
                ...
                if (InitApplication.CurrentLangId == LanguageIds.Russian)
                {
                    ...
                    ws.Range["B2"].Value = "Перечень эмитентов , аннулировавших выпуски исламских ценных бумаг, за период c " + dateBegin.ToShortDateString() + " г. по " + dateEnd.ToShortDateString() + " г.";
                ...
                {
                    ws.Range["B2"].Font.Bold = true;
                    ws.Range["B2"].Value = "Ислам бағалы қағаздар шығарылымды жойған заңды тулғалар тізімі " + dateBegin.ToShortDateString() + "ж. - " + dateEnd.ToShortDateString() + "ж.";
                    ws.Range["B2", "K2"].MergeCells = true;
                    ws.Range["B2"].HorizontalAlignment = Microsoft.Office.Interop.Excel.XlHAlign.xlHAlignLeft;
                }
                ...
                for (var i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    (rngDataRow.Cells[rowIndex, 10] as Range).Value2 = Convert.ToInt32(dt.Rows[i]["AMOUNT_ISSUE"]);
                    (rngDataRow.Cells[rowIndex, 11] as Range).Value2 = Convert.ToInt32(dt.Rows[i]["COUNT_IS"]);
