...
namespace ARM_User.ServiceLayer.Reporting.Excel
{
     ...
     * 71.	Форма И-16. Состояние выпуска исламских ценных бумаг по состоянию на
     */
    public class RepFormI16 : ReportToExcel
    {
        #region Fields
        ...
        public RepFormI16(RepForm form, LanguageIds language)
        {
            this.form = form;
        ...
        public RepFormI16(RepForm form, LanguageIds language, DateTime dateBegin)
            : this(form, language)
        {
        ...
        public System.Data.DataTable Getcursor()
        {
            ...
            using (var cmd = ConnectionHolder.Instance.Connection.CreateCommand())
            {
                ...
                cmd.CommandText = "G_REP_FORM_I16";
        ...
        protected virtual void CreateReportFromXml()
        {
            ...
            try
            {
                ...
                if (InitApplication.CurrentLangId == LanguageIds.Russian)
                    ws.Range["A2"].Value = "Форма И-16. Состояние выпуска исламских ценных бумаг по состоянию на " + dateBegin.ToShortDateString() + " г.";
                else if (InitApplication.CurrentLangId == LanguageIds.Kazakh)
                    ws.Range["A2"].Value = dateBegin.ToShortDateString() + " ж. жағдайы бойынша ислам бағалы қағаздар шығарылымдардың күйi ";
                ...
                for (var i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    (rngDataRow.Cells[rowIndex, 8] as Range).Value2 = dt.Rows[i]["AMOUNT_ISSUE"].ToString();
                    (rngDataRow.Cells[rowIndex, 9] as Range).Value2 = dt.Rows[i]["COUNT_ICB"].ToString();
                    (rngDataRow.Cells[rowIndex, 10] as Range).Value2 = dt.Rows[i]["COUNT_PL"].ToString();
                    (rngDataRow.Cells[rowIndex, 11] as Range).Value2 = dt.Rows[i]["COUNT_REDEEMED"].ToString();
                    (rngDataRow.Cells[rowIndex, 12] as Range).Value2 = dt.Rows[i]["PERCENT"].ToString();
                    (rngDataRow.Cells[rowIndex, 13] as Range).Value2 = dt.Rows[i]["NAME_STS"].ToString();
                    (rngDataRow.Cells[rowIndex, 14] as Range).Value2 = dt.Rows[i]["DATE_STS"].ToString();
                    (rngDataRow.Cells[rowIndex, 15] as Range).Value2 = dt.Rows[i]["COMMENTS_REPAYMENT"].ToString();
                    (rngDataRow.Cells[rowIndex, 16] as Range).Value2 = dt.Rows[i]["COMMENTS_REDEMPTION"].ToString();
                    (rngDataRow.Cells[rowIndex, 17] as Range).Value2 = dt.Rows[i]["EVENTS"].ToString();
                    (rngDataRow.Cells[rowIndex, 18] as Range).Value2 = dt.Rows[i]["COMMENTS"].ToString();
